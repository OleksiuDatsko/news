        ...
        try:
            ...
            users = {
                "premium": user_repo.create({
                    ...
                    "preferences": {"favorite_categories": ["tehnologii", "ekonomika"]},
                }),
                ...
                "tech": user_repo.create({
                    ...
                    "password": hashed_user_password,
                    "preferences": {"dailyDigest": True, "breakingNews": True, "favorite_categories": ["tehnologii", "ekonomika"]},
                }),
                "politics": user_repo.create({
                    "email": "politics@news.com",
                    "username": "PoliticsReader",
                    "password": hashed_user_password,
                    "preferences": {"dailyDigest": False, "breakingNews": True, "favorite_categories": ["politika"]},
                }),
                "sports": user_repo.create({
                    "email": "sport@news.com",
                    "username": "SportsFan",
                    "password": hashed_user_password,
                    "preferences": {"dailyDigest": True, "breakingNews": False, "favorite_categories": ["sport"]},
                }),
                "culture": user_repo.create({
                    "email": "culture@news.com",
                    "username": "CultureLover",
                    "password": hashed_user_password,
                    "preferences": {"dailyDigest": True, "breakingNews": True, "favorite_categories": ["kultura", "podorozhi"]},
                }),
                "business": user_repo.create({
                    "email": "business@news.com",
                    "username": "BusinessAnalyst",
                    "password": hashed_user_password,
                    "preferences": {"favorite_categories": ["ekonomika", "politika"]},
                }),
                "curious": user_repo.create({
                    "email": "curious@news.com",
                    "username": "CuriousReader",
                    "password": hashed_user_password,
                    "preferences": {"favorite_categories": ["tehnologii", "kultura", "podorozhi"]},
                }),
                "news_addict": user_repo.create({
                    "email": "newsaddict@news.com",
                    "username": "NewsAddict",
                    "password": hashed_user_password,
                    "preferences": {"breakingNews": True, "favorite_categories": ["politika", "sport", "ekonomika"]},
                }),
                ...
                "Anna": user_repo.create({
                    ...
                    "username": "Anna_K",
                    "password": hashed_user_password,
                    "preferences": {"favorite_categories": ["kultura", "podorozhi"]},
                }),
                ...
                "Dmytro": user_repo.create({
                    ...
                    "username": "Dmytro_S",
                    "password": hashed_user_password,
                    "preferences": {"favorite_categories": ["politika", "ekonomika"]},
                }),
            ...
            sub_repo.subscribe_user(user_id=users["premium"].id, plan_id=premium_plan.id)
            ...
            sub_repo.subscribe_user(user_id=users["culture"].id, plan_id=premium_plan.id)
            sub_repo.subscribe_user(user_id=users["business"].id, plan_id=corporate_plan.id)
            sub_repo.subscribe_user(user_id=users["curious"].id, plan_id=student_plan.id)
            sub_repo.subscribe_user(user_id=users["news_addict"].id, plan_id=premium_plan.id)
            ...
            sub_repo.subscribe_user(user_id=users["Dmytro"].id, plan_id=corporate_plan.id)
            ...
            users["Anna"].followed_authors.extend([authors["culture_1"], authors["culture_2"]])
            users["Ivan"].followed_authors.extend([authors["sport_1"], authors["tech_1"]])
            users["Mariya"].followed_authors.append(authors["economy_1"])
            users["Dmytro"].followed_authors.extend([authors["politics_1"], authors["economy_1"]])
            users["Olga"].followed_authors.extend([authors["culture_1"], authors["sport_1"]])
            ...
            for idx in interactions_map["tech"][:8]:
                interactions_to_add.append(
                    ArticleInteraction(user_id=users["tech"].id, article_id=articles[idx].id, interaction_type="like")
                )
            interactions_to_add.append(
                ArticleInteraction(user_id=users["tech"].id, article_id=articles[10].id, interaction_type="saved")
            )
            interactions_to_add.append(
                ArticleInteraction(user_id=users["tech"].id, article_id=articles[14].id, interaction_type="saved")
            )

            # PoliticsReader - Free
            for idx in interactions_map["politics"][:5]:
                interactions_to_add.append(
                    ArticleInteraction(user_id=users["politics"].id, article_id=articles[idx].id, interaction_type="like")
                )

            # SportsFan - Student
            for idx in interactions_map["sports"][:7]:
                interactions_to_add.append(
                    ArticleInteraction(user_id=users["sports"].id, article_id=articles[idx].id, interaction_type="like")
                )
            interactions_to_add.append(
                ArticleInteraction(user_id=users["sports"].id, article_id=articles[20].id, interaction_type="saved")
            )

            # CultureLover - Premium
            for idx in interactions_map["culture"]:
                interactions_to_add.append(
                    ArticleInteraction(user_id=users["culture"].id, article_id=articles[idx].id, interaction_type="like")
                )
            for idx in interactions_map["travel"][:4]:
                interactions_to_add.append(
                    ArticleInteraction(user_id=users["culture"].id, article_id=articles[idx].id, interaction_type="saved")
                )

            # BusinessAnalyst - Corporate
            for idx in interactions_map["economy"]:
                interactions_to_add.append(
                    ArticleInteraction(user_id=users["business"].id, article_id=articles[idx].id, interaction_type="like")
                )
            ...
            for idx in [10, 11, 12, 40, 41, 42, 48, 49, 58, 59]:
                interactions_to_add.append(
                    ArticleInteraction(user_id=users["curious"].id, article_id=articles[idx].id, interaction_type="like")
                )
            interactions_to_add.append(
                ArticleInteraction(user_id=users["curious"].id, article_id=articles[42].id, interaction_type="saved")
            )

            # NewsAddict - Premium
            for idx in [0, 20, 30, 40, 1, 21, 31, 41]:
                interactions_to_add.append(
                    ArticleInteraction(user_id=users["news_addict"].id, article_id=articles[idx].id, interaction_type="like")
                )

            # Free, Premium users та інші
            for idx in [0, 20, 48, 49]:
                interactions_to_add.append(
                    ArticleInteraction(user_id=users["free"].id, article_id=articles[idx].id, interaction_type="like")
                )

            for idx in [0, 10, 40, 41]:
                interactions_to_add.append(
                    ArticleInteraction(user_id=users["premium"].id, article_id=articles[idx].id, interaction_type="like")
                )
                interactions_to_add.append(
                    ArticleInteraction(user_id=users["premium"].id, article_id=articles[idx].id, interaction_type="saved")
                )

            # Anna, Ivan, Mariya, etc.
            for idx in interactions_map["culture"]:
                interactions_to_add.append(
                    ArticleInteraction(user_id=users["Anna"].id, article_id=articles[idx].id, interaction_type="like")
                )

            for idx in interactions_map["tech"][:5]:
                interactions_to_add.append(
                    ArticleInteraction(user_id=users["Ivan"].id, article_id=articles[idx].id, interaction_type="like")
                )
            ...
            for idx in interactions_map["economy"][:6]:
                interactions_to_add.append(
                    ArticleInteraction(user_id=users["Mariya"].id, article_id=articles[idx].id, interaction_type="like")
                )

            for idx in [0, 1, 2, 30, 31]:
                interactions_to_add.append(
                    ArticleInteraction(user_id=users["Dmytro"].id, article_id=articles[idx].id, interaction_type="like")
                )

            for idx in [40, 41, 20, 21]:
                interactions_to_add.append(
                    ArticleInteraction(user_id=users["Olga"].id, article_id=articles[idx].id, interaction_type="like")
                )
            ...
            comments_data = [
                ...
                (0, users["politics"].id, "Дуже цікава стаття! Дякую за глибокий аналіз."),
                (0, users["premium"].id, "Написано добре, але не згоден з пунктом про енергетичну безпеку. Мені здається, тут є ризики."),
                (1, users["politics"].id, "Цифри не збігаються з моїми спостереженнями. Де ви брали дані?"),
                (2, users["Dmytro"].id, "Найчастіше прочитаний матеріал про дипломатію. Браво!"),
                (3, users["business"].id, "Реформа дійсно змінює ситуацію, але потужні олігархи все ще чинять опір."),

                # Коментарі до спортивних статей
                (20, users["sports"].id, "Це була неймовірна гра! Я був на стадіоні, емоції просто зашкалюють! Наші хлопці молодці!"),
                (20, users["casual"].id, "Кращий матч, який я коли-небудь бачив. Фізичні, вольові, стратегічні аспекти – все на найвищому рівні."),
                (21, users["Ivan"].id, "Молода зірка дійсно грає приголомшливо добре. Має чемпіонський потенціал."),
                (22, users["news_addict"].id, "Якщо це правда про договіри, це найбільший скандал у спорту цього року!"),
                (29, users["sports"].id, "E-спорт – це не спорт! Люди крутять мишками на дивані!"),
                (29, users["casual"].id, "@SportsFan Ви помиляєтесь. Це вимагає таланту, стратегії та здібностей. Точно як шахи."),

                # Коментарі до економічних статей
                (30, users["business"].id, "Економіка буде стабільна, якщо центробанк утримуватиме курс."),
                (31, users["casual"].id, "Мої друзі з малим бізнесом дійсно страждають від інфляції. Грустна реальність."),
                (34, users["Mariya"].id, "Фондовий ринок буде і надалі волатильним. Краще тримати готівку!"),
                (38, users["business"].id, "Робота майбутнього – це буде новий рівень зайнятості для освічених людей."),

                # Коментарі до туристичних статей
                (48, users["culture"].id, "Був на Боржаві минулого літа, це дійсно космос! Чорниці можна їсти просто з куща годинами :)"),
                (48, users["Anna"].id, "Дякую за ідеї! Про Криворівню не знав, обов'язково заїду наступного разу."),
                (49, users["culture"].id, "Львів – це така красива та історична місто! Кожен кут розповідає історію."),
                (50, users["curious"].id, "10 лайфхаків – дійсно корисні! Уже заплановую подорож до Бенгалії."),
                (51, users["Anna"].id, "Мальдіви вибираємо! Туди краще літати. Сейшели – це дорого і далеко."),

                # Коментарі до культурних статей
                (40, users["culture"].id, "NFT – це майбутнє мистецтва чи просто спекуляція?"),
                (41, users["Anna"].id, "Виставка була справді авангардна! Деякі твори мене шокували."),
                (42, users["culture"].id, "Крутий фільм! Режисер – справжній художник. Чекаю наступного проекту!"),
                (43, users["curious"].id, "Українське кіно дійсно набирає популярність. Гордимося!"),
                (45, users["Anna"].id, "Театр – це душа культури. Віртуальна реальність не замінить живої емоції."),

                # Коментарі до здоров'я
                (54, users["news_addict"].id, "Вакцина безпечна? Я все ще маю сумніви..."),
                (54, users["premium"].id, "Дослідження показує, що вакцина ефективна на 95%. Довіряйте науці!"),
                (55, users["casual"].id, "Стрес – це основна проблема 21 століття. Потребуємо більше часу для релаксації."),
                (56, users["Anna"].id, "Фітнес тренди мінливі. Найважливіше – знайти те, що вам подобається!"),

                # Коментарі до технологій
                (10, users["tech"].id, "ШІ дійсно революціонізує все! Але потребуємо етичних норм."),
                (11, users["curious"].id, "Стартапи з Гарварду створюють майбутнє прямо зараз. Вразливо!"),
                (12, users["tech"].id, "Квантові комп'ютери – це чи наступний крок людства, чи дорога до катастрофи?"),
                (15, users["tech"].id, "Чекаю, коли батарея буде тримати хоча б 8 годин. До того – це просто іграшка для багатіїв."),
                ...
                (1, users["news_addict"].id, "Не згоден з попереднім коментатором. Важливо ходити на вибори. Дякую за аналітику опитувань."),
                (4, users["casual"].id, "Найтруднішим буде переглад всієї системи судочинства."),
            ...
            ads_data = [
                {"title": "Знижки на техніку", "ad_type": "banner", "impressions": 5000, "clicks": 150},
                {"title": "Курс з Python", "ad_type": "sidebar", "impressions": 12000, "clicks": 250},
                {"title": "Доставка їжі 'Смаколик'", "ad_type": "sidebar", "impressions": 8000, "clicks": 180},
                {"title": "Нова колекція одягу", "ad_type": "inline", "impressions": 9500, "clicks": 210},
                {"title": "Онлайн-кінотеатр 'KinoGo'", "ad_type": "video", "impressions": 25000, "clicks": 1200},
                {"title": "Квитки на концерт 'Ocean'", "ad_type": "popup", "impressions": 3000, "clicks": 450},
                {"title": "Страхування авто 'Надійно'", "ad_type": "banner", "impressions": 6000, "clicks": 90},
                {"title": "Курси англійської 'SpeakUp'", "ad_type": "sidebar", "impressions": 11000, "clicks": 320},
                {"title": "Спортивне харчування", "ad_type": "inline", "impressions": 7000, "clicks": 280},
                {"title": "Подорож до Єгипту", "ad_type": "banner", "impressions": 15000, "clicks": 600},
                {"title": "Новий смартфон 'Pixel 9'", "ad_type": "video", "impressions": 18000, "clicks": 950},
                {"title": "Юридичні послуги 'Право'", "ad_type": "sidebar", "impressions": 2000, "clicks": 15, "active": False},
                {"title": "Фітнес-клуб 'SportLife'", "ad_type": "popup", "impressions": 4500, "clicks": 700},
                {"title": "Ремонт квартир 'Майстер'", "ad_type": "inline", "impressions": 5500, "clicks": 110},
                {"title": "Книгарня 'Літера'", "ad_type": "banner", "impressions": 8500, "clicks": 310},
                {"title": "Вебінар з маркетингу", "ad_type": "sidebar", "impressions": 6200, "clicks": 420},
                {"title": "Кава 'Gourmet Beans'", "ad_type": "inline", "impressions": 4300, "clicks": 130},
                {"title": "Ігровий монітор 'ViewMax'", "ad_type": "video", "impressions": 16000, "clicks": 880},
                {"title": "Підпишіться на розсилку!", "ad_type": "popup", "impressions": 10000, "clicks": 1500},
                {"title": "Еко-товари 'Zeleno'", "ad_type": "sidebar", "impressions": 7100, "clicks": 190},
                {"title": "Ветеринарна клініка 'ДоброЛап'", "ad_type": "banner", "impressions": 3900, "clicks": 120},
                {"title": "Йога-студія 'Гармонія'", "ad_type": "inline", "impressions": 5100, "clicks": 220},
                {"title": "Хмарне сховище 'CloudDrive'", "ad_type": "sidebar", "impressions": 13000, "clicks": 410},
                {"title": "Альбом гурту 'Stray'", "ad_type": "video", "impressions": 22000, "clicks": 1100},
            ]
