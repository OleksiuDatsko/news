    ...
    try {
      ...
      userStore.update((user) => {
        ...
        if (isFollowing) {
          ...
          user.followed_authors = favs.filter((id) => id !== data.author.id);
        }
        return { ...user };
      });
    } catch (e: any) {
...
<svelte:head>
  <title>
    {data.author ? `${data.author.first_name} ${data.author.last_name}` : 'Автор не знайдений'}
  </title>
  {#if data.author?.bio}
  ...
  {#if data.author}
    ...
    {:else}
      <div class="bg-white p-12 rounded-lg shadow-md text-center">
        <p class="text-gray-600 text-lg">У цього автора ще немає опублікованих статей.</p>
      </div>
    {/if}
  {:else}
    <div class="bg-white p-12 rounded-lg shadow-md text-center">
