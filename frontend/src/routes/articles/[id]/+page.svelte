  ...
  $effect(() => {
    if (article) {
      ...
      if (!sessionStorage.getItem(key)) {
        api
          .post(`/articles/${article.id}/impression`, {})
          .then(() => {
    ...
    try {
      ...
      userStore.update((user) => {
        ...
        if (isSaved) {
          ...
          user.savedArticles = savedArticles.filter((a) => a !== article.id);
        }
        return user;
      });
    } catch (error) {
...
{#if article}
  <div class={`grid ${data.ads && data.ads.length > 0 ? 'grid-cols-[2fr_1fr] gap-8' : ''}`}>
    <div class="bg-white rounded-lg shadow-lg p-6 md:p-10">
      ...
      {#if data.recommendations && data.recommendations.length > 0}
        <div class="mt-12 border-t pt-8">
          <h2 class="text-2xl font-bold mb-6">Рекомендовані статті</h2>
          <div class="space-y-6">
    ...
    {#if data.ads && data.ads.length > 0}
      <aside class="m-4">
        <div class="sticky top-24 space-y-6">
          <h3 class="text-xl font-bold text-gray-900 border-b pb-2">Реклама</h3>
...
{:else if data.error?.status == 403}
  <div class="bg-white p-12 rounded-lg shadow-md text-center">
    ...
    <a href="/" class="text-indigo-600 hover:underline mt-4 block">Повернутися на головну</a>
  </div>
{:else}
  <div class="bg-white p-12 rounded-lg shadow-md text-center">
    <h1 class="text-2xl font-bold text-red-600">Помилка</h1>
    <p class="text-gray-600 mt-4">Не вдалося завантажити статтю. Можливо, її не існує.</p>
    <a href="/" class="text-indigo-600 hover:underline mt-4 block">Повернутися на головну</a>
  </div>
{/if}
