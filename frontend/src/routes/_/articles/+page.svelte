<script lang="ts">
  ...
  async function handlePublish(articleId: number) {
    ...
    error = '';
    try {
      const updatedArticle = await api.put<IArticle>(`/admin/articles/${articleId}/status`, {
        status: 'published'
      });
...
<div class="bg-white shadow-lg rounded-lg overflow-hidden">
              ...
              <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-2">
                {#if article.status === 'draft'}
                  <Button
                    ...
                    loading={loadingPublishId === article.id}
                    disabled={loadingAction || loadingPublishId !== null}
  ...
  {:else}
    <div class="text-center p-12">
      ...
      <h3 class="mt-2 text-lg font-medium text-gray-900">Жодної статті не знайдено</h3>
      <p class="mt-1 text-sm text-gray-500">Почніть наповнювати ваш сайт контентом.</p>
      <div class="mt-6">
