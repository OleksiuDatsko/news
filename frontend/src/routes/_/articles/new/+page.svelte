<script lang="ts">
  ...
  let title = $state('');
  let content = $state('');
  let author_id = $state<number | null>(null);
  let category_id = $state<number | null>(null);
  let status = $state('draft');
  let is_exclusive = $state(false);
  let is_breaking = $state(false);
...
<svelte:head>
  <title>Адмін: Нова Стаття</title>
</svelte:head>

<h1 class="text-3xl font-bold text-gray-900 mb-6">Створити Нову Статтю</h1>
...
<div class="bg-white p-8 rounded-lg shadow-lg max-w-4xl mx-auto">
  <form onsubmit={handleSubmit} class="space-y-6">
    ...
    <div>
      <label for="content" class="block text-sm font-medium text-gray-700"> Вміст статті </label>
      <div class="mt-1">
        <textarea
          id="content"
          name="content"
    ...
    <div class="flex items-center gap-6">
      <div class="flex items-center">
        ...
        />
        <label for="is_exclusive" class="ml-2 block text-sm text-gray-900"> Ексклюзив </label>
      </div>
      <div class="flex items-center">
        <input
          id="is_breaking"
          ...
          class="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded"
        />
        <label for="is_breaking" class="ml-2 block text-sm text-gray-900"> Термінова новина </label>
      </div>
    </div>

    <div class="flex gap-4">
      <Button type="submit" {loading} class="!w-auto">Створити Статтю</Button>
      <a
        href="/_/articles"
