<script lang="ts">
	...
	let categories = $state<ICategoryAdmin[]>(data.categories);
	let error = $state('');
	...
	async function handleDelete(categoryId: number) {
		...
		if (
			!confirm(
				'Ви впевнені, що хочете видалити цю категорію? Це неможливо, якщо у неї є статті.'
			)
		) {
			return;
		}
...
<div class="bg-white shadow-lg rounded-lg overflow-hidden">
			...
			<table class="min-w-full divide-y divide-gray-200">
				<thead class="bg-gray-50">
					<tr>
						...
						<th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">К-ть статей</th>
				...
				<tbody class="bg-white divide-y divide-gray-200">
					{#each categories as category (category.id)}
						<tr class="hover:bg-gray-50 transition-colors duration-150">
							...
							<td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{category.articles_count ?? 0}</td>
	...
	{:else}
		<div class="text-center p-12">
			...
			<div class="mt-6">
				<Button onclick={handleCreateNew} class="!w-auto shadow-sm"> Створити першу категорію </Button>
			</div>
		</div>
	{/if}
</div>