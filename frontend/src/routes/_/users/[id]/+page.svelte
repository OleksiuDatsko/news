<script lang="ts">
	...
	let currentPlanName = $derived(
		data.user?.current_subscription?.plan.name ?? 'Відсутня'
	);
	...
	<div class="grid grid-cols-1 md:grid-cols-2 gap-8">
		<div class="bg-white p-8 rounded-lg shadow-lg">
			...
			<form onsubmit={handleProfileSubmit} class="space-y-6">
				...
				<Button type="submit" loading={profileLoading} class="!w-auto">
					Зберегти Зміни
				</Button>
			</form>
		</div>
...
{:else}
	...
	<Alert type="error">
		Не вдалося завантажити дані. Можливо, користувача було видалено.
	</Alert>
