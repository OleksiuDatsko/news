...
<div class="max-w-4xl mx-auto">
  <h1 class="text-3xl font-bold text-gray-900 mb-4">Результати пошуку</h1>
  ...
  <form onsubmit={handleFilterSubmit} class="bg-white p-6 rounded-lg shadow-md mb-8 space-y-4">
    ...
    <Button type="submit" class="!w-auto">Застосувати фільтри</Button>
  </form>
  {#if data.total > 0}
  ...
  {:else if data.query}
    <div class="bg-white p-12 rounded-lg shadow-md text-center">
      <p class="text-gray-600 text-lg">На жаль, за вашим запитом нічого не знайдено.</p>
  ...
  {:else}
    <div class="bg-white p-12 rounded-lg shadow-md text-center">
      <p class="text-gray-600 text-lg">Будь ласка, введіть пошуковий запит, щоб почати.</p>
    </div>
  {/if}
</div>
