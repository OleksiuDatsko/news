<script lang="ts">
	...
	async function handleSubmit() {
		...
		error = '';
		try {
			const { user } = await api.post<{user: IUser}>('/auth/login', { email, password });
			userStore.set(user);
			await goto('/');
		} catch (e: any) {
...
<FormWrapper title="Вхід у акаунт" description="Увійдіть, щоб отримати доступ до всіх можливостей.">
	...
	<p class="mt-4 text-center text-sm text-gray-600">
		Немає акаунту? <a href="/auth/register" class="font-medium text-indigo-600 hover:text-indigo-500">Зареєструватися</a>
	</p>
</FormWrapper>