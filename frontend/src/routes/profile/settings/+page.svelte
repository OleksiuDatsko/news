<script lang="ts">
  ...
  let isSubscribedNewsletter = $state($userStore?.is_subscribed_to_newsletter ?? false);
  ...
  async function handleToggleNewsletter() {
    ...
    try {
      ...
      newsletterError = e.message || 'Не вдалося оновити підписку на розсилку.';
  ...
  <div class="bg-white p-8 rounded-lg shadow-lg border border-gray-100">
    <form onsubmit={handleSubmit} class="space-y-6">
      ...
      <fieldset class="space-y-4 pt-4">
        <legend class="text-lg font-semibold text-gray-800 border-b pb-2">Email-розсилка</legend>
        {#if newsletterError}
      ...
      <fieldset class="space-y-4">
        <legend class="text-lg font-semibold text-gray-800 border-b pb-2">Push-сповіщення</legend>
        <div class="flex items-center justify-between">
