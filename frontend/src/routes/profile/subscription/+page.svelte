<script lang="ts">
  ...
  async function handleSubscribe(planId: number) {
    ...
    try {
      const newSubscription = await api.post<any>('/subscriptions/subscribe', { plan_id: planId });
...
<div class="max-w-4xl mx-auto">
  ...
  <h1 class="text-3xl font-bold text-gray-900 mt-2 mb-8">Керування підпискою</h1>
  ...
  <div class="bg-white p-6 rounded-lg shadow-lg border border-gray-100 mb-8">
    ...
    {:else}
      <p class="text-gray-600">У вас немає активної підписки.</p>
    {/if}
  </div>
      ...
      {#each data.plans as plan (plan.id)}
        ...
        >
          ...
          <p class="text-2xl font-bold text-gray-800 my-3">
            {Math.round(plan.price_per_month)} грн<span class="text-sm font-normal text-gray-500"
              >/міс</span
            >
          </p>
